...
class AccountDetailScreen extends StatefulWidget {
  ...
  const AccountDetailScreen({super.key, required this.accountId, required this.accountName});
...
class _AccountDetailScreenState extends State<AccountDetailScreen> {
  ...
  Future<void> _load() async {
    ...
    if (!mounted) return;
    setState(() { _account = accounts.firstWhere((a) => a.id == widget.accountId, orElse: () => LocalAccount(id: "", name: "")); _loading = false; });
  }
  ...
  Future<void> _deleteKeychain(Keychain keychain) async {
    final ok = await showDialog<bool>(
      context: context, builder: (_) => AlertDialog(
        title: const Text("删除模型串"),
        content: Text("确定删除「${keychain.name}」及其中所有模型？"),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text("取消")),
          FilledButton(onPressed: () => Navigator.pop(context, true),
              style: FilledButton.styleFrom(backgroundColor: AromaticTheme.error), child: const Text("删除")),
        ],
      ),
    );
    if (ok == true && mounted) { await _service.deleteKeychain(widget.accountId, keychain.id); await _load(); }
  }

  Future<void> _addKey(Keychain keychain) async {
    final result = await showDialog<ApiKey>(context: context, builder: (_) => const AddKeyDialog());
  ...
  Future<void> _editKey(Keychain keychain, ApiKey key) async {
    final result = await showDialog<ApiKey>(context: context, builder: (_) => AddKeyDialog(existing: key));
    if (result == null || !mounted) return;
  ...
  Future<void> _deleteKey(Keychain keychain, ApiKey key) async {
    final ok = await showDialog<bool>(
      context: context, builder: (_) => AlertDialog(
        ...
        content: Text("确定删除「${key.modelName}」？"),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text("取消")),
          FilledButton(onPressed: () => Navigator.pop(context, true),
              style: FilledButton.styleFrom(backgroundColor: AromaticTheme.error), child: const Text("删除")),
    ...
    if (ok == true && mounted) { await _service.deleteKey(widget.accountId, keychain.id, key.id); await _load(); }
  }
              ...
              child: Center(
                child: SizedBox(
                  ...
                  child: _loading
                      ...
                      : _account == null
                          ? const Center(child: Text("账户未找到"))
                          : ListView(
                              padding: const EdgeInsets.all(AromaticTheme.spaceMD),
                              children: [for (final kc in _account!.keychains) _buildKeychainCard(context, kc, colors, isDark)],
                            ),
                ),
              ),
            ),
  ...
  Widget _buildKeychainCard(BuildContext context, Keychain keychain, AromaticColors colors, bool isDark) {
    return GlassCard(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisSize: MainAxisSize.min, children: [
        Row(children: [
          ...
          GestureDetector(
            ...
            child: Text(keychain.name, style: Theme.of(context).textTheme.titleMedium),
          ),
          const Spacer(),
          Text("${keychain.keys.length} 个模型", style: Theme.of(context).textTheme.labelSmall),
          const SizedBox(width: 4),
          InkWell(
            onTap: () => _deleteKeychain(keychain), borderRadius: BorderRadius.circular(16),
        ...
        if (keychain.keys.isEmpty) ...[
          ...
          Text("模型串为空，添加第一个模型", style: Theme.of(context).textTheme.bodyMedium),
        ] else
          ...keychain.keys.map((key) => _buildKeyTile(context, keychain, key, colors)),
        const SizedBox(height: AromaticTheme.spaceMD),
        Center(
          child: OutlinedButton.icon(
            onPressed: () => _addKey(keychain), icon: const Icon(Icons.add, size: 16),
  ...
  Widget _buildKeyTile(BuildContext context, Keychain keychain, ApiKey key, AromaticColors colors) {
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(key.modelName, style: TextStyle(fontSize: 14, color: colors.textPrimary)),
          Text("${key.format.label} · ${key.modelId}", style: TextStyle(fontSize: 12, color: colors.textMuted)),
        ])),
        InkWell(onTap: () => _editKey(keychain, key), borderRadius: BorderRadius.circular(16),
            child: Padding(padding: const EdgeInsets.all(6),
                child: Icon(Icons.edit_outlined, size: 16, color: colors.textMuted))),
        InkWell(onTap: () => _deleteKey(keychain, key), borderRadius: BorderRadius.circular(16),
            child: Padding(padding: const EdgeInsets.all(6),
                child: Icon(Icons.delete_outline, size: 16, color: colors.textMuted))),
      ]),
    );
